...
export default function RTable(props: RTableProps) {
    ...
    return (
        <div className={classNames?.table ?? ''}>
            ...
            >
                <select defaultValue={take} onChange={(event) => {
                ...
                <span>{(take * page) + 1}-{rows.length < take ? rows.length : ((take * page) + take)} {of} {rows.length}</span>
                <div>
                    {pagination?.previousPage ? pagination?.previousPage :
                        <button disabled={page === 0} className={pagination?.previousPageClassName ?? ''}>
                ...
                <div>
                    {pagination.nextPage ? pagination.nextPage :
                        <button disabled={page === 0} className={pagination?.nextPageClassName ?? ''}>
